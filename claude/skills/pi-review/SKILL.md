---
name: pi-review
description: "Structured code review with prioritized findings, fail-fast error handling checks, and human callouts. Supports uncommitted changes, branch diffs, commits, PRs, and folder snapshots. Use when user wants a thorough code review."
---

# Code Review

Structured code review inspired by [earendil-works/pi-review](https://github.com/earendil-works/pi-review).

## Invocation

```
/pi-review                              → review uncommitted changes (default)
/pi-review uncommitted                  → review uncommitted changes
/pi-review branch <name>                → diff against base branch
/pi-review commit <sha>                 → review a specific commit
/pi-review pr <number-or-url>           → review a GitHub pull request
/pi-review folder <path> [<path>...]    → snapshot review (not a diff)
```

Any mode accepts `--extra "..."` to add a focus area (e.g. `--extra "focus on error handling"`).

## Step 1 — Gather the diff

Based on the mode, collect the material to review:

- **uncommitted**: Run `git diff` (staged + unstaged) and `git diff --cached` and `git ls-files --others --exclude-standard` to capture all changes including untracked files.
- **branch \<name\>**: Run `git merge-base HEAD <name>` (try upstream first via `git rev-parse --abbrev-ref "<name>@{upstream}"`), then `git diff <merge-base>`.
- **commit \<sha\>**: Run `git show <sha>` or `git diff <sha>~1 <sha>`.
- **pr \<ref\>**: Parse the PR number (or extract from GitHub URL). Run `gh pr diff <number>`. If the diff is large, use `gh pr view <number> --json files` to list changed files and read them selectively.
- **folder \<paths\>**: No diff — read the files/directories directly. This is a snapshot review.

If no arguments are given, default to **uncommitted**. If there are no uncommitted changes, fall back to reviewing the current branch against the default branch.

## Step 2 — Load project review guidelines

Check if a `REVIEW_GUIDELINES.md` file exists at the project root. If it does, read it and append its contents to the review as additional project-specific instructions. These override the general guidelines below where they conflict.

## Step 3 — Perform the review

Apply the review rubric below to the gathered material. Read referenced files for full context when the diff alone is ambiguous.

## Step 4 — Output findings

Use the output format specified in the rubric. Every review must end with the Human Reviewer Callouts section.
...
Below are default guidelines for determining what to flag. If you encounter more specific guidelines in the project's `REVIEW_GUIDELINES.md` or in the `--extra` instruction, those override these general instructions.
...
5. Flag tautological tests: tests that cannot fail if the code under test is broken. Examples: asserting a mock returns the value it was configured to return, re-computing the expected value with the same logic as the implementation, asserting a constant or type equals itself, or asserting only that a function was called without checking its effect.
...
7. End with the required "Human Reviewer Callouts (Non-Blocking)" section.
...
After findings/verdict, ALWAYS append:
